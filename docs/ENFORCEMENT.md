...
| Comparative evaluation records | `tests/eval/runner.py`: refuses a drifted frozen input by name, an unpinned model, an absent spend ceiling, an unmetered campaign and a deadline raised above the one the case records; pins isolation flags per surface; records per-cell status, usage, cost and blinded scores. `tests/test_eval_runner.py` covers hashing, blinding determinism, refusals, timeout handling and resume | Establishes evidence for criterion 5 on the named surface and version only. Not promotion, not semantic acceptance, not a sandbox. Isolation flags are recorded as passed, not verified as honoured. The candidate arm supplies skill text as a prompt, so it does not establish native discovery or loading. Grading is human judgment against a frozen rubric; structural blinding is not independence |
