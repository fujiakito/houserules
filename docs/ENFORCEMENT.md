...
| An evaluation scenario with headroom | The v2 rubric is saturated: checks 1-3 scored 1.0 for both arms across all twenty gradings in two independent runs, the [2026-09-16 replication](../tests/workflows/prior-art/v2/attempt-20260916-replication.json) and the leak-free [2026-09-17 replication](../tests/workflows/prior-art/v2/attempt-20260917-replication.json), so it cannot resolve a small effect in either direction. No pending decision currently depends on that measurement | A handoff or artifact-consumption failure observed in real work, creating a decision that needs the instrument |
