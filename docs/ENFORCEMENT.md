...
| Comparative evaluation records | `tests/eval/runner.py`: refuses a drifted frozen input by name, an unpinned model, an absent spend ceiling, an unmetered campaign, a deadline raised above the one the case records, a manifest missing a hash for a declared input, a resume under changed settings or a changed CLI version, and grading an output whose hash changed or while a run holds the attempt lock; pins isolation flags per surface and runs each cell in a fresh empty directory outside the repository; judges attempt status against the recorded full-arm plan; records per-cell status, usage, cost and blinded scores. `tests/test_eval_runner.py` covers hashing, blinding determinism, refusals, timeout handling, resume and the working directory | Establishes evidence for criterion 5 on the named surface and version only. Not promotion, not semantic acceptance, not a sandbox. Isolation flags are recorded as passed, not verified as honoured; user-level context is not proven excluded. The candidate arm supplies skill text as a prompt, so it does not establish native discovery or loading. Grading is human judgment against a frozen rubric; structural blinding is not independence |
