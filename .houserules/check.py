...
In a repository that installed this layer the script lives at `.houserules/check.py`. --repo
resolves against the current directory, not against this file, so run it from the project root
or pass --repo explicitly - running it from inside .houserules/ would check that directory.
...
# The one opt-in asset outside .houserules/ besides HOUSERULES.md: a CI provider only reads
# workflows from its own directory. Once selected it is retained like any other selection.
CI_WORKFLOW = ".github/workflows/houserules.yml"
# Recording this path is also what distinguishes an installation made after the checker moved
# from one made before it: a manifest without this entry was written by a pre-move installer.
INSTALLED_CHECKER = ".houserules/check.py"
ASSET_PATHS = {"HOUSERULES.md", ".houserules/LICENSE", ".houserules/START.md",
               INSTALLED_CHECKER, ".houserules/workflow.py",
               ".houserules/work/README.md", CI_WORKFLOW} | {
...
def main() -> int:
    ...
    ap.add_argument("--repo", default=".",
                    help="repository to check (default: the current directory, not this file's location)")
