#!/usr/bin/env python3
"""Frozen v2 paired trial - replication with counterbalancing, isolated blind grading and
opaque working-directory names.
...
Differences from the 2026-09-16 single-pair harness (attempt-20260916-outputs/harness.py):
  ...
  5. Every call's working directory is an opaque random token. The 2026-09-16 harnesses named arm
     directories `cwd-baseline` / `pNN-baseline`, and Claude Code headless places the working
     directory in the model's context (verified: a `-p` call with no tools returns its own cwd
     verbatim), so each generating call could read a word naming its own arm. Directory names now
     carry no arm, order or label information; the token -> call mapping lives in the record.
...
def main() -> int:
    ...
    for i in range(1, N_PAIRS + 1):
        ...
        for arm in order:
            lab = "p%02d-%s" % (i, arm)
            token = rng.randbytes(6).hex()
            rec = run_one(lab, prompts[arm], os.path.join(BASE, "cwd", token))
            rec["cwd_token"] = token
            rec["arm"] = arm
            rec["pair"] = i
        ...
        for lab, arm in mapping.items():
            ...
            gtoken = rng.randbytes(6).hex()
            grec = run_one(glab, gprompt, os.path.join(BASE, "cwd", gtoken))
            grec["cwd_token"] = gtoken
